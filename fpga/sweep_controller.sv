...
//     // FSM States
//     typedef enum logic [2:0] {IDLE, WAIT_MCU, SET_FREQ, MCU_DONE, NEXT_FREQ, DONE} statetype;
...
module sweep_controller # (
    ...
    always_comb begin
        case (state)
            ...
            NEXT_FREQ: begin
                if (phase_inc_reg >= PHASE_INC_MAX) begin  // If we've reached max
                    nextstate = DONE;                       // We're done! 
                end else begin                              // Otherwise
                    nextstate = WAIT_MCU;                   // Continue sweeping
                end
            end
            DONE: begin
                nextstate = DONE;  // Stay in DONE until reset
